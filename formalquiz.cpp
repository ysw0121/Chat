...
void FormalQuiz::countdown(){ 
           ...
           if(time.second()==0&&time.msec()==0){
               ...
               if(mode==1){
                   ...
                   succeed1.times++;
                   if(succeed1.correct<score){
                       succeed1.correct=score;
                       succeed1.time=30.000;
                   }
                   else if(succeed1.correct==score){
                       if(succeed1.time>30.000)succeed1.time=30.000;
                   }

                   if(succeed1.times==1)succeed1.time=30.000;

                   for(int i=0;i<usrlist.size();i++){
                       if(succeed1.name==usrlist[i].name){
                           usrlist[i]=succeed1;
                           break;
                       }
                   }

                   QFile file1("game_user.txt");
    ...
    if(time.second()!=0){
         ...
         if(num==num_of_ex){
             ...
             if(mode==1){
                 ...
                 if(succeed1.correct<score){
                     succeed1.correct=score;
                     succeed1.time=resultTime;
                 }

                 else if(succeed1.correct==score){
                     if(succeed1.time>resultTime)succeed1.time=resultTime;
                 }

                 if(succeed1.times==1)succeed1.time=resultTime;
